...
socket.on("connect", () => {
  ...
  messageInput.placeholder = `Message #${localStorage.getItem("currentChannel")}`
...
$(document).on("click", ".channel-link", function (e) {
  ...
  socket.emit("query channels")
  socket.emit("query users")
  socket.emit("query messages", this.dataset.channel, (date = new Date().toLocaleString()))
})
...
messageForm.addEventListener("submit", e => {
  ...
  socket.emit("add message", newMessage, localStorage.getItem("currentChannel"))
})
